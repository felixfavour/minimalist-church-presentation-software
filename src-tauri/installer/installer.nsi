; Cloud of Worship installer.
;
; Derived from the Tauri bundler's stock NSIS template at tauri-bundler-v2.9.3.
; Every deviation is marked with a "CoW:" comment. When bumping Tauri, re-fetch
; the upstream template and re-apply those blocks:
;   https://github.com/tauri-apps/tauri/blob/tauri-bundler-v2.9.3/crates/tauri-bundler/src/bundle/windows/nsis/installer.nsi
;
; The bitmaps this references are generated by ./generate-assets.mjs.
...
; 1. Welcome Page
;
; CoW: the welcome page is our whole brand moment. SIDEBARIMAGE is a full
...
Function CoWWelcomeShow
  ...
  ; Size from the page's own client rect rather than a hardcoded 499x314, so the
  ; bitmap still covers the page when Windows is scaling above 100%.
...
!if "${LICENSE}" != ""
  !define MUI_PAGE_CUSTOMFUNCTION_PRE Skip ; CoW: no wizard steps
...
; CoW: no finish page. The installer closes itself and launches the app, so the
; desktop shortcut and the app launch move into the install section below.
...
Section Install
  ...
  ; CoW: the finish page (and its shortcut checkbox) is skipped in every mode,
  ; so always create the desktop shortcut.
  ...
  ; CoW: always auto close, there is no finish page to fall through to.
  SetAutoClose true
SectionEnd
