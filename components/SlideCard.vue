<template>
  <!-- GRID TYPE CARD -->
  <div v-if="gridType"
    class="slide-card flex items-center justify-center text-left gap-3 p-2 h-[120px] border border-gray-100 rounded-md bg-primary hover:bg-primary-700 transition-all cursor-pointer relative overflow-hidden">
    ...
    <!-- DELETE SLIDE BUTTON -->
    <ConfirmDialog class="absolute bottom-2 right-2" button-icon="i-bx-trash" header="Delete slide"
  ...
  <button v-else
    ...
    :class="{ 'bg-red-100': live }" @click="$emit('click')">
    <div class="slide-preview w-24 min-w-24 h-16 bg-primary-600 text-white rounded-md overflow-hidden sm-preview">
      <LiveContent :slide="slide" padding="0" />
    </div>
