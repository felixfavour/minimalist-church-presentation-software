<template>
  ...
  <div v-if="gridType"
    ...
    <button :class="['transition-all', { 'opacity-70 ': selectable }]" @click="$emit('click')">
      ...
      <div class="overlay-gradient absolute inset-0" :class="{ 'border-4 border-primary': selected }"></div>
      <div class="texts flex items-center gap-2 text-white absolute top-1 right-2 left-2">
    ...
    <div class="actions absolute bottom-2 right-2 flex gap-1">
      <UTooltip v-if="slide.type === slideTypes.text" text="Duplicate Slide">
        <UButton icon="i-bx-copy" size="xs" variant="ghost" class="px-1.5 text-white hover:bg-primary-500"
          @click.stop.prevent="$emit('duplicate')">
        </UButton>
      </UTooltip>

      <ConfirmDialog
        v-if="slide?.type === slideTypes.text || slide?.type === slideTypes.media || slide?.type === slideTypes.song"
        button-icon="i-bx-save" header="Save to Library" button-styles="px-1.5 text-white hover:bg-primary"
      ...
      <ConfirmDialog button-icon="i-bx-trash" header="Delete slide" button-styles="px-1.5 text-white hover:bg-primary"
        label="Are you sure you want to delete this slide? This action is not reversible"
        @confirm="$emit('delete', slide?.id)">
      </ConfirmDialog>
    </div>
    <div v-if="selectable" class="selectable-actions absolute bottom-4 right-3 flex gap-1">
      <UCheckbox name="select" :model-value="checkboxSelected" :ui="{ base: 'h-8 w-8' }"
  ...
  <button v-else
    ...
    :id="slide?.id" @click="appStore.setLiveSlide(slide?.id || '0')">
